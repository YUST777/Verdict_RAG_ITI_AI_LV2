...
export default function ProblemDrawer({
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            // Step 1: Try the browser cache (the stripped app has no account API).
            try {
                const raw = window.localStorage.getItem(`verdict-sheet-${currentContestId}`);
                const cachedSheet = raw ? JSON.parse(raw) as ActiveSheet : null;
                if (cachedSheet?.problems?.length) {
                    if (!cancelled) {
    ...
    const loadContestProblems = useCallback(async (
        ...
        try {
            ...
            if (data.status === "OK" && data.result?.problems) {
                ...
                try {
                    window.localStorage.setItem(`verdict-sheet-${contestId}`, JSON.stringify(newSheet));
                } catch {
                    // Storage is optional; the in-memory drawer remains usable.
                }
            }
        } catch (err) {

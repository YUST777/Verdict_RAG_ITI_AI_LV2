...
const LETTER_ICONS: Record<string, TabIcon> = {
    A: FileCode,
    B: FileText,
    C: FileCog,
    D: FileSearch,
...
export function SidebarTabs({ isOpen, onClose, currentUrl }: SidebarTabsProps) {
    ...
    useEffect(() => {
        ...
        const fetchTabs = async () => {
            setLoading(true);
            try {
                const raw = window.localStorage.getItem('verdict-local-tabs');
                setTabs(raw ? JSON.parse(raw) : []);
            } catch (err) {
                console.error(err);
            } finally {
                setLoading(false);
    ...
    const saveTabs = async (newTabs: TabData[]) => {
        setTabs(newTabs);
        window.localStorage.setItem('verdict-local-tabs', JSON.stringify(newTabs));
    };
                ...
                {loading ? (
                    ...
                    <>
                        {tabs.map((tab) => {
                            ...
                            const IconComponent: TabIcon = LETTER_ICONS[iconLetter] || FileCode2;
